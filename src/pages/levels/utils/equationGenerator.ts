...
const generateUniqueEquation = (settings: LevelSettings): EquationConfig => {
    ...
    // New case: If we have value_1 and empty answer string
    if (settings.value_1 && settings.value_2 && settings.answer === '' && settings.answer_required === 'answer') {
        // First evaluate value_1
        const value1 = evaluateValueWithDependencies(settings.value_1, evaluatedValues);
        evaluatedValues['V1'] = value1;

        // If value_2 depends on V1, use that value
        let value2;
        if (settings.value_2 === '@V1') {
            value2 = value1;
        } else if (settings.value_2) {
            value2 = evaluateValueWithDependencies(settings.value_2, evaluatedValues);
        } else {
            // Generate a random value between 0-5 if no value_2 specified
        ...
        return {
            numbers,
            operators: [operation === 'addition' ? '+' : '-'],
            result: operation === 'addition' ? value1 + value2 : value1 - value2,
            show_sum: settings.show_sum ?? true,
            answer_required: 'answer',
        };
    }
